...
class Controller_Workorders extends Controller_Account {
    ...
    public function action_generate() {
        ...
        if ($status) {
            if (Model_Workorders::$type === Model_Workorders::EXPERT_INSPECTION) {
                $this->request->redirect('/assets/pdf/reports/'. str_replace("'", "", str_replace(' ', '', $workorder_info->last_name)) . "_Claim" . str_replace(' ', '', $workorder_info->policy_number) . ".pdf");
            } else {
                $this->request->redirect('/assets/pdf/reports/'. str_replace("'", "", str_replace(' ', '', $workorder_info->last_name)) . "_Claim" . str_replace(' ', '', $workorder_info->policy_number) . ".pdf");
            }
        } else {
    ...
    public function action_send() { 
        ...
        $status = $this->mailer_model->send_mail($workorder_info->adjuster_email, 'admin@trinity.is', 'Inspection Report for work order id : ' . $this->_workorder_id, 15, 
